...
<script lang="ts">
	...
	// Removed Drizzle-specific type; using a generic type for chat.
	// Replace "any" with your preferred Chat type.
	let {
		chat,
		class: c,
	}: { chat: any; class?: import('svelte/elements').ClassValue } = $props();
	...
	const { label, Icon } = $derived(
		(chatFromHistory &&
			visibilities.find((v) => v.id === chatFromHistory.visibility)) ??
			visibilities[0],
	);
</script>
